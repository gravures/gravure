...
class fromiterTest(unittest.TestCase):
    ...
    def test04a(self):
        ...
        N = 10*1000
        a = np.fromiter((i*2 for i in range(N)), dtype='f8')
        b = ca.fromiter((i*2 for i in range(N)), dtype='f8', count=len(a))
        assert_array_equal(b[:], a, "iterator with a hint fails")

    def test04b(self):
        """Testing fromiter method with large iterator with a hint"""
        N = 10*1000
        a = np.fromiter((i*2 for i in range(N)), dtype='f8', count=N)
        b = ca.fromiter((i*2 for i in range(N)), dtype='f8', count=N)
        assert_array_equal(b[:], a, "iterator with a hint fails")
...
class evalTest(MayBeDiskTest):
    ...
    def test08(self):
        ...
        a, b = list(range(int(self.N))), list(range(int(self.N)))
...
class dtypesTest(unittest.TestCase):
    ...
    def test05(self):
        ...
        a = np.array(["aŀle", "eñe", "açò"], dtype="U4")
