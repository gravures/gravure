...
#-----------------------------------------------------------------
#NOTE: Python 3 correctif here____________________________________
from cpython.buffer cimport PyObject_CheckBuffer, PyObject_GetBuffer
cdef extern from *:
...
cdef object PyString_FromStringAndSize(char *v, Py_ssize_t length):
    cdef bytes r
    r = <bytes> v[:length]
    return r

cdef char* PyString_AsString(object string) except NULL:
    cdef char* data_ptr = NULL
    cdef Py_buffer info

    if PyObject_CheckBuffer(string):
        PyObject_GetBuffer(string, &info, PyBUF_SIMPLE)
        data_ptr = <char*> info.buf

    return data_ptr

#NOTE: end of correctif____________________________________________
# Blosc routines
...
cdef class carray:
  ...
  def write_meta(self):
      ...
      with open(storagef, 'wb') as storagefh:
        ...
        storagefh.write(json.dumps(tmp, ensure_ascii=True).encode('ascii'))
        storagefh.write(b"\n")
  ...
  def trim(self, object nitems):
    ...
    if not isinstance(nitems, (int, float)):
  ...
  def __iter__(self):
    ...
    if not (self.sss_mode or self.where_mode or self.wheretrue_mode):
      self.nhits = 0
      self.limit = sys.maxsize #sys.maxint
      self.skip = 0
