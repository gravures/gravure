# -*- coding: utf-8 -*-
#
# Copyright (C) 2011 Gilles Coissac.
# Authors: Gilles Coissac <gilles@atelierobscur.org>
#
...
__notimplemented__= [
    'acosh', 'asinh', 'atanh', 'erf', 'erfc', 'expm1', 'fabs', 'factorial',
...
__todo__ = [
    'copy_abs', 'copy_negate', 'imag', 'ln', 'logb', 'max', 'min', 'normalize'
]
...
class Context(_decimal.Context):
    ...
    Inherits from decimal.context.

    :param prec: precision (for use in rounding, division, square roots..)
    :param rounding: rounding type (how you round)
    :param traps: If traps[exception] = 1, then the exception is raised when
                  it is caused.  Otherwise, a value is substituted in.
    ...
    :param angle: Define unit of mesurement for angle in trigonometric functions.
                  should be gravure.ANGLE.RADIAN or gravure.ANGLE.DEGREE,
                  default use radian.
    ...
    @angle.setter
    def angle(self, a):
        self.__property['angle'] = a if a in [
            gravure.ANGLE.DEGREE, gravure.ANGLE.RADIAN
        ] else gravure.ANGLE.RADIAN
    ...
    def _shallow_copy(self):
        ...
        return Context(
            self.prec, self.rounding,  self.Emin, self.Emax, self.capitals,
            self.clamp, self.flags, self.traps, self.angle, self.Dfraction,
        )
    ...
    def copy(self):
        ...
        return Context(
            self.prec, self.rounding, self.Emin, self.Emax, self.capitals,
            self.clamp, self.flags.copy(), self.traps.copy(), self.angle,
            self.Dfraction
        )
    __copy__ = copy

    def __repr__(self):
        """Return the context string representation."""
        flags = ', '.join([f.__name__ for f, v in self.flags.items() if v])
        traps = ', '.join([t.__name__ for t, v in self.traps.items() if v])
        angle = ['gravure.ANGLE.RADIAN', 'gravure.ANGLE.DEGREE'][self.angle]
        return (
            f"Context(prec={self.prec}, rounding={self.rounding}, "
            f"Emin={self.Emin}, Emax={self.Emax}, capitals={self.capitals}, "
            f"clamp={self.clamp}, flags=[{flags}], traps=[{traps}], "
            f"angle={angle}, Dfraction={bool(self.Dfraction)})"
        )
...
def __to_GContext(c):
    return Context(
        c.prec, c.rounding, c.Emin, c.Emax, c.capitals,
        c.clamp, c.flags.copy(), c.traps.copy()
    )
