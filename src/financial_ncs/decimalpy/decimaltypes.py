...
class PolyExponents(SortedKeysDecimalValuedDict):
    ...
    * exponent: 0, factor: 1
    * exponent: 1, factor: 5
    * exponent: 2, factor: 10
    * exponent: 3, factor: 10
    * exponent: 4, factor: 5
    * exponent: 5, factor: 1
    ...
    * variable_index=1
    * args=(Data for the PolyExponents:
    * exponent: 0, factor: 2
    * exponent: 1, factor: 2
    ...
    * argument_is_decimal=False
    ...
    def __str__(self):
        content = '\n* '.join(sorted(['exponent: %s, factor: %s' % (key, value)
                                    for key, value in self.items()]))
        return 'Data for the %s:\n* %s' % (self.__class__.__name__, content)

    __repr__ = __str__   # TODO so Latex
    ...
    def __call__(self, base_value):
        ...
        exponent1, factor1 = self.items()[0]
        for exponent2, factor2 in self.items()[1:]:
            factor1 = decimal_simple_poly_exponent(base_value,
                                                   exponent1 - exponent2,
                                                   factor1,
                                                   factor2
                                                   )
